...
**голосом** — как делает Яндекс.Браузер.

Два режима озвучки:

| Движок              | Качество             | Скорость           | Что нужно                        |
|---------------------|----------------------|--------------------|-----------------------------------|
| **Браузер** (Web Speech) | системное TTS    | мгновенно          | ничего                           |
| **Бэкенд** (XTTS-v2)     | клонирует тембр  | ~5–10× медленнее RT на CPU | свой FastAPI-сервис, см. [`backend/`](backend/) |

«Бэкенд» делает всё end-to-end: yt-dlp скачивает аудио, Faster-Whisper
распознаёт английский, Google Translate переводит, XTTS-v2 синтезирует
русскую речь **с тембром оригинального спикера** (нужен 6-секундный
фрагмент, который выбирается автоматически из самого длинного сегмента).
